...
int main()
{
    /*12. Escribir un programa que lea del teclado una cadena y construya, y muestre en pantalla,
otra cadena en la que cada vocal haya sido reemplazada por un punto.*/
    char cadena[50];
    ...
    for(int i = 0; i < strlen(cadena); i++){ // strlen(cadena) para leer SOLO lo que el usuario escribió
        if(isalpha(cadena[i])){ // Verifico si el caracter actual es una letra del alfabeto
            ...
            if(esvocal==1){ // Evaluo si la bandera esta levantada es porque es vocal
                cadena[i] = '.';
            }
        }
    }

    printf("\nLa cadena fue modicada y las vocales fueron reemplazadas por puntos!");
    printf("\nAhora la cadena es esta: \n %s", cadena);
    return 0;
}
